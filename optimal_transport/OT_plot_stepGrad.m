...
%with dE^(1) && homogeneous Dirichlet b.c
...
%%%n=8, k=3 (Dirichlet)
alpha_n8_3  = alpha_list;
quotient_n8_3 = [0.9945,0.9945,0.9945,0.9945,0.9945,0.9944,0.9943,0.9940,0.9936,0.9932,0.9928,0.9925,0.9922,0.9918,0.9912,0.9905,0.9892, ...
    0.9878,0.9865,0.9851,0.9838,0.9825,0.9812,0.9785,0.9746,0.9681,0.9554,0.9430,0.9309,0.9191,0.9076,0.8963,0.8854,0.8748,0.8445, ...
    0.8256,0.8078,0.7827,0.7597,0.7525,0.7455,0.7386];
...
%%% (Dirichlet)
quotient_n16_3 = [1.0107,1.0107,1.0107,1.0107,1.0106,1.0106,1.0104,1.0101,1.0097,1.0093,1.0088,1.0086,1.0082,1.0078,1.0071,1.0064,  ...
    1.0050,1.0035,1.0021,1.0007,0.9993,0.9979,0.9965,0.9936,0.9894,0.9825,0.9689,0.9557,0.9428,0.9301,0.9178,0.9058,0.8942,0.8828, ...
    0.8503,0.8300,0.8107,0.7837,0.7587,0.7509,0.7433,0.7358];


%%%n=32, k=3 (Dirichlet)
alpha_n32_3  = alpha_list;
quotient_n32_3 = [1.0038,1.0038,1.0037,1.0037,1.0037,1.0036,1.0035,1.0032,1.0028,1.0023,1.0019,1.0016,1.0012, ...
    1.0009,1.0001,0.9994,0.9980,0.9965,0.9951,0.9937,0.99220,0.9908,0.9894,0.9865,0.9823,0.9753,0.9616,0.9483,0.9353, ...
    0.9226,0.9102,0.8982,0.8864,0.8750,0.8424,0.8221,0.8029,0.7759,0.7512,0.7434,0.7358,0.7284];


%%%n=42, k=3 (Dirichlet)
%alpha_n42_3  = alpha_list;

quotient_n42_3 = [1.002260, 1.002253, 1.002245, 1.002216, 1.002173, 1.002129, 1.001983, 1.001692, 1.001255, 1.000819, ...
    1.000383, 1.000092, 0.999729, 0.999366, 0.998642, 0.997918, 0.996473, 0.995031, 0.993594, 0.992159, 0.990729, 0.989302 ...
    0.987878, 0.985042,0.9808,0.9738,0.9601,0.9468,0.9338,0.9211,0.9087,0.8967,0.8850,0.8735,0.8410,0.8207,0.8015,0.7746,0.7498,0.7421, ...
    0.7345,0.7271];

alpha_n42_3  = alpha_list(1:length(quotient_n42_3));
...
% hold on
semilogy(alpha_n8_3, quotient_n8_3, 'lineStyle',':', 'linewidth', 1, 'Marker', all_marks{1}, 'markersize', 4) 
hold on
semilogy(alpha_n16_3, quotient_n16_3, 'lineStyle','--', 'linewidth', 1, 'Marker', all_marks{2}, 'markersize', 4) 
hold on
semilogy(alpha_n32_3, quotient_n32_3, 'lineStyle','-.', 'linewidth', 1, 'Marker', all_marks{3}, 'markersize', 4) 
hold on
semilogy(alpha_n42_3, quotient_n42_3, 'lineStyle','-', 'linewidth', 1, 'Marker', all_marks{5}, 'markersize', 4) 

%yline(1, '--', 'linewidth', 1);

legend('$\mathbf{P}_3$, $n=8^2$', '$\mathbf{P}_3$, $n=16^2$', '$\mathbf{P}_3$, $n=32^2$', ...
    '$\mathbf{P}_3$, $n=42^2$', 'interpreter','latex', 'location', 'best')


grid on
% legend('$\mathbf{P}_4$, $n=6^2$ mailles', '$\mathbf{P}_4$, $n=8^2$ mailles', '$\mathbf{P}_4$, $n=10^2$ mailles', ...
%     '$\mathbf{P}_3$, $n=16^2$ mailles', '$\mathbf{P}_3$, $n=32^2$ mailles', ...
